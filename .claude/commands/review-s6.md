# Section 6: チーム開発 コードレビューオーケストレーター

Description: Section 6（チーム開発）の課題リポジトリに対して、共通レビュー＋セクション固有レビューを実行し、REVIEW_REPORT.md を生成します。

## 重要なルール

- メインエージェント（このコマンド）はソースコード・Gitログを**直接読み込まない**こと（コンテキスト汚染防止）。
- 各フェーズで必ずサブエージェントを使用すること。
- **サブエージェントには `model: "haiku"` を指定すること**（高速化のため）。
- レビューガイドライン: `~/.claude/resources/review_guidelines.md`
- 教材リファレンス: `~/.claude/resources/sections/s6/materials.md`
...
4. **教材の存在確認**:
   ```bash
   ls ~/.claude/resources/sections/s6/materials.md 2>/dev/null
   ```
...
## Phase 1 & 2: 共通レビュー + セクション固有レビュー（並列実行）

**Phase 1 と Phase 2 のサブエージェントを同時に起動する。** 直列実行しないこと。

### Phase 1: 共通レビュー

サブエージェントを起動し（`model: "haiku"`）、以下の指示を与える:
...
> **簡潔なチェックリスト形式で出力すること。コード例は原則不要。**

### Phase 2: セクション固有レビュー

サブエージェントを起動し（`model: "haiku"`）、以下の指示を与える:

> `~/.claude/agents/review_s6_team.md` を読み、その指示に従ってSection 6固有のレビューを実行してください。
> レビューガイドラインは `~/.claude/resources/review_guidelines.md` を参照してください。
> 教材リファレンスは `~/.claude/resources/sections/s6/materials.md` を参照してください。
> gh CLI の可用性: {Phase 0 の結果}
> 出力は review_s6_team.md に定義されたフォーマットに従ってください。
> **簡潔なチェックリスト形式で出力すること。コード例は原則不要。**

両方の出力をそれぞれ **共通レビュー結果** と **セクション固有レビュー結果** として保持する。
