...
- Git・開発プロセス評価結果（G1-G4: 4項目）※Issue管理はNotionのため除外
...
対象項目一覧:
- G1, G2, G3, G4（4項目）※Issue管理はNotionのため除外
...
- G2（PRレビュー）が [NG] なのに D4（PR記述）が [OK]
...
- [OK] が 20項目以上（26項目中）: 「**POSSIBLY_TOO_LENIENT**」— 本当に全て基準を満たしているか再確認を推奨
- [NG] が 18項目以上（26項目中）: 「**POSSIBLY_TOO_STRICT**」— 部分的にできている項目が [WARN] ではないか再確認を推奨
...
- gh CLI が使用できなかった場合、影響を受ける項目（G2, G3, D4）に制約が記載され、確信度が LOW になっているか
