...
レビュー出力は **簡潔さを最優先** とする。受講生に「何ができていて、何ができていないか」をシンプルに伝える。

- **良い点**: 3-5項目、各1-2行（file:line + なぜ良いか）
- **改善提案**: 3-8項目、各2-3行（現状 → 提案 → 理由を各1文）
- **コード例は原則省略**（必要な場合のみ最小限）
- **チェックリスト形式**（✅/⚠️/❌）を積極活用
