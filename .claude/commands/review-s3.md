# Section 3: React + Next.js コードレビューオーケストレーター

Description: Section 3（React + Next.js）の課題リポジトリに対して、共通レビュー＋セクション固有レビューを実行し、REVIEW_REPORT.md を生成します。
...
- 教材リファレンス: `~/.claude/resources/sections/s3/materials.md`
...
4. **教材の存在確認**:
   ```bash
   ls ~/.claude/resources/sections/s3/materials.md 2>/dev/null
   ```
...
> `~/.claude/agents/review_s3_react_next.md` を読み、その指示に従ってSection 3固有のレビューを実行してください。
> レビューガイドラインは `~/.claude/resources/review_guidelines.md` を参照してください。
> 教材リファレンスは `~/.claude/resources/sections/s3/materials.md` を参照してください。
> 出力は review_s3_react_next.md に定義されたフォーマットに従ってください。
